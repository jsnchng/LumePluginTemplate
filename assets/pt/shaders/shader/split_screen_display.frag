...
layout(set = 0, binding = 1) uniform texture2D uPredictedColorOutput;
...
void main(void)
{
    ...
    if (uv.y < 0.5) {
        // Top row
        if (uv.x < 0.5) {
            // Top-left: predicted color output
            texUv = vec2(uv.x * 2.0, uv.y * 2.0);
            outColor = textureLod(sampler2D(uPredictedColorOutput, uSampler), texUv, 0);
        } else {
